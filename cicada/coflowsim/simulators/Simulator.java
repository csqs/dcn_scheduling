...
import coflowsim.datastructures.RackNode;//csqs
...
public abstract class Simulator {
  ...
  protected Vector<RackNode> netRacks;//csqs
  protected int finishedJob;//csqs
  ...
  protected void initialize(TraceProducer traceProducer) {
    ...
    this.netRacks = new Vector<RackNode>();//csqs
    this.finishedJob = 0;//csqs
    for (int i = 0; i < NUM_RACKS; i++) {
      flowsInRacks[i] = new Vector<Flow>();
      netRacks.add(new RackNode(i));//csqs
    }
    ...
    for (CURRENT_TIME = 0; CURRENT_TIME < Constants.SIMULATION_ENDTIME_MILLIS
      ...
      for (long i = 0; i < EPOCH_IN_MILLIS; i += Constants.SIMULATION_QUANTA) {
        ...
        if (curTime % Constants.SIMULATION_SECOND_MILLIS == 0) {
          System.err.printf("Timestep %6d: Running: %3d Started: %5d Total %3d Finished %3d\n",
              (curTime / Constants.SIMULATION_SECOND_MILLIS), numActiveJobs, curJob, TOTAL_JOBS, finishedJob);
        }
