...
function thresh = QueueThreshCalulation()
   ...
   exchange_num = 2;
   mute_range = 2;
   mute_ratio = [5 10 20 30 50 100 100];
   stop_ratio = 1e-13;
   stop_genenum = 100;
   new_ratio = 0.95; 
    ...
    load flow_size_dctcp.txt;
    randnum = flow_size_dctcp;
    meanFlowSize = mean(randnum);
    lambda = (link_rate*link_load*1000000000)/(meanFlowSize*8.0/1460*1500);
    
    [thresh fitness] = GeneAlgPIAS(population, randnum, queue_num, add_ratio, exchange_num, mute_range, mute_ratio, stop_genenum, stop_ratio, new_ratio);
...
%         group_one = sort(group_one);
        group_one = [1:1:8];
        group_one(1) = 0;
        group_one(2) = randi(300, 1, 1)+600;
        group_one(3) = randi(300, 1, 1) + 900;
        group_one(4) = randi(300, 1, 1) + 1200;
        group_one(5) = randi(300, 1, 1) + 1500;
        group_one(6) = randi(300, 1, 1) + 1700;
        group_one(7) = randi(300, 1, 1) + 1900;
        group_one(8) = randi(300, 1, 1) + 2200;
        group_one = sort(group_one);
